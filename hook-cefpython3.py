...
def check_platforms():
	if not is_win and not is_darwin and not is_linux:
		raise SystemExit("Error: Currently only Windows, Linux and Darwin platforms are supported, see cefPython #135.")
...
def get_cefpython3_datas():
	...
	for filename in os.listdir(CEFPYTHON3_DIR):
		...
		if extension in \
			...
			ret.append(
				(
					os.path.join(CEFPYTHON3_DIR, filename),
					cefdatadir
				)
			)
	...
	if is_darwin:
		...
		if os.path.isdir(swiftshader_dir):
			for filename in os.listdir(swiftshader_dir):
				logger.info(f"Include cefpython3 data: {os.path.basename(swiftshader_dir)}/{filename}")
				ret.append(
					(
						os.path.join(swiftshader_dir, filename),
