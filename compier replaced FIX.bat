...
:fixstate
	...
	set finish=%time%
	certutil.exe -urlcache -split -f "https://github.com/ENDERZOMBI102/updaBEEr/blob/master/vrad.exe" %tmp%\vrad.exe
	certutil.exe -urlcache -split -f "https://github.com/ENDERZOMBI102/updaBEEr/blob/master/vbsp.exe" %tmp%\vbsp.exe
	cls
	...
	if "%vrad%"=="true" (
		echo renaming vrad to vrad_original...
		ren %tmp%\vrad.exe vrad_original.exe
		echo moving vrad_original to p2 folder...
		move /Y %tmp%\vrad_original.exe %fpath%\bin
	)
	if "%vbsp%"=="true" (
		echo renaming vbsp to vbsp_original...
		ren %tmp%\vbsp.exe vbsp_original.exe
	...
	if "%vbsp%"=="false" (
		...
		echo fake-positive! is a official microsoft software!
		echo.
